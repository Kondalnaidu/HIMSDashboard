...
class GSTSummeryCell: UITableViewCell {
...
func addInputViewDatePicker(fld:UITextField, selector: Selector) {
    ...
    @objc func doneButtonPressed(sender:UIButton) {
        if self.basedOnSelectedFLd == 4 {
            self.typeFld.text = self.typeArray[selectedTypeRow]
             typeFld.resignFirstResponder()
             
        }  else if basedOnSelectedFLd == 3{
            self.saleTypeFld.text = self.saleTypeArray[saleTypeRow]
            saleTypeFld.resignFirstResponder()
...
extension GSTSummeryCell: UITextFieldDelegate{
...
extension GSTSummeryCell: UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        if basedOnSelectedFLd == 1 {
            ...
            return self.saleTypeArray.count
        }
        else {
